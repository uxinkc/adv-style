...
var steppedControl__elements = document.querySelectorAll('.adv-stepped-control--sticky');
...
function steppedControl__setStyle() {
  ...
  Helper.forEach(steppedControl__elements, function(index, value) {
    ...
    if (_scBottomPosition > 12) {
      ...
      if(!_el.classList.contains('adv-stepped-control--unstuck')){
        _el.classList.add('adv-stepped-control--unstuck');
      }
    } else {

      if(_el.classList.contains('adv-stepped-control--unstuck')){
        _el.classList.remove('adv-stepped-control--unstuck');
      }
    }
  });
}
...
if(steppedControl__elements.length){
  ...
  var steppedControl__modals = document.querySelectorAll('.adv-modal');
